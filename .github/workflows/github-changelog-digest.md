...
- リアクションは URL の閉じ括弧 `)` の後ろに続く文字列だけを対象にし、前後の余分な空白を除いて保持してください。
- `📍👀` のように複数のアイコンが連続している場合も、順序を変えずに 1 つの行末リアクションとして保持してください。
...
- タイトルが変わっても URL が同じなら引き継いでください。
- タイトルが同じでも URL が違う場合は引き継がないでください。
